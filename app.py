...
if raw_df.empty:
    ...
    if not f_df.empty:
        ...
        # ==========================================
        # TAB 1: Market Review (包含总体配比和月度数字表)
        # ==========================================
        with tab1:
            ...
            with col_m1:
                ...
                fig_line = px.line(trend_df, x='sale_date', y='sold_qty', color='category', markers=True, 
                                   color_discrete_sequence=[BRAND_COLOR, DARK_COLOR, "#64748b", "#cbd5e1"])
            ...
            if not crm_df.empty:
                ...
                for client in client_pivot.index[:12]: 
                    with st.expander(f"🏢 {client} | Detailed Purchase Matrix"):
                        ...
                        # 💡 核心改动：Model 在 index (左侧)，Period 在 columns (上方)
